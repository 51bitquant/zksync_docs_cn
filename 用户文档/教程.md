...
在本教程中，我们将使用 MetaMask 讲资金存入 zkSync 钱包。

> 虽然本教程 MetaMask 中显示的是 Rinkeby 测试网图像，但你可以按照与主网相同的步骤进行操作
...
<br>
要将你的钱包连接到 zkSync 1.0 主网，请选择 MetaMask 顶部的"以太坊主网"按钮。
...
<br>
你可以选择不同的方法将资金添加到你的 zkSync 钱包，包括 FIAT onramps、Exchanges 或 Bridges。
...
- 如果你找不到你的代币：
  - 一些代币在我们的[代币页面](https://zkscan.io/explorer/tokens/)上通过它们的内部
    ID 来引用而不是他们的代币符号（例如 ERC20-23 = UNI））
  - 将代币添加到 zkSync：
    1. 点击“找不到代币？
...
<br>
如果添加需要批准的代币，则需要在以太坊上支付 Gas 费，以授权存款到 zkSync。在继续执行步骤 4 之前，你将看到以下消息。 
...
5. 这是L1交易（当你将资金从 L1 存入 L2
   ...
   处理你的交易后，你的资金才会出现在 L2 上。如果你有任何困难，请检查 
...
6. 你的存款已完成。交易已开始，你的资金将在你的 L1 交易十次确认后显示在你的
   zkSync 钱包中。

| **已提交**                                                                                                                                          | **已验证**                                                                                                                                       |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| 你余额旁边的一个黄色复选标记表示 zkSync 服务器已处理你的交易并且你的资金已准备好并立即使用。 | 两个绿色复选标记表示包含你交易的区块证明已经生成并提交给第 1 层的智能合约。 |
...
1. 你将被提示“授权签名帐户激活”，需要先对消息进行签名以生成你的 zkSync L2 私钥。
